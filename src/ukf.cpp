...
void UKF::Prediction(MatrixXd sig, double delta_t) {
  ...
  for (int i = 0; i < sig_n; i++) {
    ...
    noise << 0.5 * delta_t * delta_t * cos(sig_col[3]) * sig_col[5],
        ...
        delta_t * sig_col[5], 0.5 * delta_t * delta_t * sig_col[6],
        delta_t * sig_col[6];
    if (fabs(yawdd) < 0.001) {
      iter_col[0] = sig_col[2] * cos(sig_col[3]) * delta_t;
      iter_col[1] = sig_col[2] * sin(sig_col[3]) * delta_t;
    } else {
      iter_col[0] = sig_col[2] / sig_col[4] *
                    (sin(sig_col[3] + sig_col[4] * delta_t) - sin(sig_col[3]));
      iter_col[1] = sig_col[2] / sig_col[4] *
                    (-cos(sig_col[3] + sig_col[4] * delta_t) + cos(sig_col[3]));
  ...
  std::cout << "XsigPred-PostPrediction: " << std::endl
            << Xsig_pred_ << std::endl;
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {
    VectorXd iter_col = Xsig_pred_.col(i);
    Zsig.col(i)[0] =
        sqrt(iter_col[0] * iter_col[0] + iter_col[1] * iter_col[1]);
    Zsig.col(i)[1] = atan2(iter_col[1], iter_col[0]);
    Zsig.col(i)[2] = (iter_col[0] * cos(iter_col[3]) * iter_col[2] +
                      iter_col[1] * sin(iter_col[3]) * iter_col[2]) /
                     Zsig.col(i)[0];
    z_pred += weights_[i] * Zsig.col(i);
  }
